...
It is clear from the distributions that the simulation results have a severe right skew. Most sites were likely situated less than a meter from the shoreline, and from this there is a decline in density as one moves further along the x-axis. This trend is reasonably approximated by a exponential function that has been fit to the distributions for vertical distance using maximum likelihood estimation (Figure \@ref(fig:results2)). An expanded consideration of this relationship and comparison with some standard models is provided in the supplementary material. While exponential decay reasonably captures this relationship, it is also clear that it does not perfectly match the data. However, this can at least in part be related to methodological factors, where the accumulation of distance-values on the 0m mark likely follow from the resolution of the spatial data, from defining intersecting sea- and site polygon as having a zero distance, from sampling error and, although they are relatively few, from forcing negative values to zero. If one accepts this, the probability density function for exponential decay can be used to characterise the vertical distance between sites and the shoreline and be used to inform a method for shoreline dating that takes this into account.
