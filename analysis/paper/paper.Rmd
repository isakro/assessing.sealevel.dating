...
```{r shoredate, echo = FALSE, fig.cap = "The result of backwards shoreline dating the sites with radiocarbon dates corresponding to the artefact inventory. The sites are dated using the mean elevation of the site polygon and by employing the exponential decay function identified in Figure 7 for characterising the distance between sites and shoreline. A) The shoreline dates in gray are plotted against the oldest radiocarbon-informed phase for each site in black. The red line indicates the 95\\% probability range for the shoreline date. B) Using the method of Parnell et al. (2008), the synchroneity of the dates is evaluated ", out.width = "525px", fig.align="center", regfloat=TRUE}
...
In Figure \@ref(fig:redate), excavated Stone Age sites within the study area where ^14^C-dates are not available or are not believed to date the main occupation of the sites are also subjected to the method for shoreline dating outlined above. These are compared to the dates originally proposed in the excavation reports for the sites. To avoid issues with the DTM, the sites are dates based on the mean elevation of those provided for each site in the reports. This is an illustrative, but unfair exercise for a few reasons. First of all the dates provided in the reports are typically stated to be a very rough estimate, and are sometimes given as a point estimate with an implied or explicit, but undefined uncertainty range. Secondly, seeing as these reports are from various dates in time, many are based on now outdated data on RSL-change. Finally, they are often only meant to indicate a lower bound for when the sites could have been in use. I do, however, think that the results indicate that shoreline dating, in general, has been applied with a fairly reasonable degree of success, seeing as these dates have typically been interpreted and informed research in an approximate manner. The results from applying the method devised here again highlights the spatial and temporal contingency of the method. In some cases the method provides a very precise date range and in others it offers little more than a *terminus post quem*.  Furthermore, a few of the date ranges extend well beyond the Stone Age and could be severely and securely constrained with only cursory reference to typology. This thus points to another way that the dating method could be improved. In the above example from Brunlanes it was shown how this could conceivably be done by evaluating the geographical location of the sites. Another possibility is of course to draw on other temporal data, such as typological indicators or radiocarbon dates with which a shoreline date could be combined, also within a Bayesian framework.     
...
It is worth mentioning some sources of likely variation and uncertainty that have not been considered here. First of all the DTM has only been corrected for major modern disturbances. This means that erosion, although likely not that prevalent, has not been taken into account. Secondly, the DTM has a vertical error [cf. @lewis2021]. Thirdly, while mean sea-level is used for dating the sites, a consideration of the tidal range
...
ges are often for example rounded off to 200-year intervals, means that most previous research based on the method is given support by the findings reported here. However, the findings also suggest that simply using the method as a terminus ante quem for the occupation of the sites is overly conservative and would stand to loose important chronological information unnecessarily. Attempting to move these considerations further through an explicit quantification has here resulted in a concrete recommendation of applying an offset when shoreline dating sites where no other information is available.
