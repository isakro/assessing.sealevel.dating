...
```{r shoredate, eval = TRUE, echo = FALSE, fig.cap = "The result of shoreline dating the analysed sites by mean elevation of the site polygon, accounting for distance to closest isobase and employing the exponential decay function identified in Figure 7 for the distance between sites and shoreline. The shoreline dates in gray are plotted against the oldest radiocarbon-informed phase for each site in black. The red line indicates the 95% probability range for the shoreline date.", out.width = "525px", fig.align="center"}
...
The pre-Boreal sites from the Brunlanes-project are among the earliest known sites in Norway. They have a distinct Early Mesolithic artefact inventory and are situated in an extremely steep area of the landscape where it would be difficult to envision any use of the sites after the sea retreated any significant distance from their location. Not least following from the fact that no pre-Boreal ^14^C-dates associated with anthropogenic activity have been achieved in Norway, the shoreline dating of these earliest sites are of fundamental importance for understanding the pioneer settlement and intial colonisation of the Scandinavian peninsula. In the original publication of the sites, Jaksland provides a thorough discussion of shoreline dating in general and as used for the dating of the Brunlanes sites specifically. The result of comparing his results against the ones achieved using the above-outlined approach are given in Figure. These are largely in agreement. The sites are located more or less on the isobase of the displacement curve for Larvik and so neglible adjustment was needed. The slight discrepancies can result from the fact that a slightly updated version of the displacement curve is applied here, as well as Jaksland applying the lowest find occurence for the shoreline dating, which is slightly off from the lowest elevation of the site limits employed here. Jaksland's dates are a given a 200 and 50 year uncertainty range starting from what he gave as the earliest possible date. The sites are located in such a constrained area of the landscape that the difference in site elevation would arguably be enough to constrain the uncertainty range so that they don't overlap between sites -- information that is not integrated here. The steepness of the curve does nonetheless provide very precise dates, the accuracy of which are of course ultimately dependent on the veracity of the geological reconstruction.  
