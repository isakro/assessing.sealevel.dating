...
This section provides some further elaboration on the choice of an exponential model to describe the vertical distance between sites and shoreline. The normal distribution has been fit when including the negative values. After this, two exponential models were fit to the distrbutions. One when forcing negative values to zero and one where these are made NA, to evaluate the relative impact of these different choices. As the differences are negligeble, the choice was made to use the first solution. This is also the to which the power law distribution has been fit. To be able to fit a power law distribution, 0.01 have been added to all values, including for the normal and exponential functions, to avoid having values of zero. All models are fit using maximum likelihood estimation. 

Although it appears evident that the exponential model is the best fit simply from inspecting the, the models have also been compared using the Schwarz criterion, often also termed the Bayesian information criteria [BIC, see e.g. @timpson2021; @crema2019 for previous use in archaeology]. This evaluates the ability of the models to describe the data while also taking into account the number of parameters in the models to avoid over-fitting. 
