...
pauler <- sitespol %>%
  filter(str_detect(site_name, 'Pauler')) %>%
  st_zm() %>%
  rename("name" = "site_name",
         "limit" = "name")

pauler <- st_join(st_make_valid(pauler), isopolys,
                    join = st_intersects, largest = TRUE)

sdatesp <- list()
for(i in 1:nrow(pauler)){
  print(pauler$name[i])
  sdatesp[[i]] <- shoreline_date_exp(sitename = pauler$name[i],
                                            dtm = dtm,
...
jaksland <- data.frame(site_name = c(sort(unique(pdates$site_name))),
                       earliest_date = c(-9200, -9150, -9000,
                                         -8950, -8975, -8850, -8800)) %>%
...
ggplot(data = hdr,
  ...
  geom_segment(data = jaksland, aes(x = earliest_date, xend = latest_date,
                                    y = site_name, yend = site_name), col = "blue", size = 1) +
  labs(y = "", x = "") +
  theme_bw()
