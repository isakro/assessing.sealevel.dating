...
rpath <- here::here("analysis/data/derived_data")
writeRaster(dmask, file.path(rpath, "dtm10.tif"),
            overwrite = TRUE)
...
execGRASS("r.in.gdal", flags = c("overwrite"), parameters = list(input = rpath,
                                                  output = "tmprast"))
...
# Save the raster
writeRaster(raster::raster(intrast), file.path(rpath, "dtm10.tif"),
            overwrite = TRUE)
